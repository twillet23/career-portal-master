describe('Job Detail', () => {
  it('Should be able to open the apply modal and apply to the job', () => {
    ...
    cy.intercept({
      ...
      query: {
        fields: 'id,title,publishedCategory(id,name),address(city,state,zip),employmentType,dateLastPublished,publicDescription,isOpen,isPublic,isDeleted,publishedZip,salary,salaryUnit,categories(id,name)',
        where: '*id=*',
      },
    }).as('jobResponse');
