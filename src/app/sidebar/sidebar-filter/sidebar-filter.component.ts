...
export class SidebarFilterComponent implements OnChanges {
  ...
  public ngOnChanges(changes: SimpleChanges): void {
    switch (this.field) {
      case 'categories(id,name)':
        this.fieldName = 'categories';
        break;
      default:
    ...
    switch (this.field) {
      ...
      case 'categories':
        ...
        .filter((unfilteredResult: ICategoryListResponse) => {
          return !!unfilteredResult.categories;
        })
        .map((result: ICategoryListResponse) => {
          return {
            value: result.categories.id,
            label: `${result.categories.name} (${result.idCount})`,
          };
        });
        interaction = (API: FieldInteractionApi) => {
          let values: string[] = [];
          ...
          values = API.getActiveValue().map((value: number) => {
            return `categories.id{?^^equals}${value}`;
          });
          }
          this.checkboxFilter.emit(values);
        };
